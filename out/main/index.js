var __create = Object.create;
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __getProtoOf = Object.getPrototypeOf;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __copyProps = (to, from, except, desc) => {
	if (from && typeof from === "object" || typeof from === "function") for (var keys = __getOwnPropNames(from), i = 0, n = keys.length, key; i < n; i++) {
		key = keys[i];
		if (!__hasOwnProp.call(to, key) && key !== except) __defProp(to, key, {
			get: ((k) => from[k]).bind(null, key),
			enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable
		});
	}
	return to;
};
var __toESM = (mod, isNodeMode, target) => (target = mod != null ? __create(__getProtoOf(mod)) : {}, __copyProps(isNodeMode || !mod || !mod.__esModule ? __defProp(target, "default", {
	value: mod,
	enumerable: true
}) : target, mod));
let __electron_toolkit_utils = require("@electron-toolkit/utils");
let electron = require("electron");
let __prisma_adapter_better_sqlite3 = require("@prisma/adapter-better-sqlite3");
let node_path = require("node:path");
node_path = __toESM(node_path);
let node_url = require("node:url");
let __prisma_client_runtime_client = require("@prisma/client/runtime/client");
__prisma_client_runtime_client = __toESM(__prisma_client_runtime_client);
let zod = require("zod");
zod = __toESM(zod);
var config = {
	"previewFeatures": [],
	"clientVersion": "7.2.0",
	"engineVersion": "0c8ef2ce45c83248ab3df073180d5eda9e8be7a3",
	"activeProvider": "sqlite",
	"inlineSchema": "datasource db {\n  provider = \"sqlite\"\n}\n\ngenerator client {\n  provider = \"prisma-client\"\n  output   = \"../generated/prisma\"\n}\n\nmodel Games {\n  id           Int           @id @default(autoincrement())\n  name         String\n  cover        String\n  status       String\n  genres       String\n  length       Int?\n  platform     String\n  reviewScore  Float?        @map(\"review_score\")\n  metadata1    String?\n  metadata2    String?\n  metadata3    String?\n  metadata4    String?\n  metadata5    String?\n  listsEntries ListEntries[]\n\n  @@index([name])\n}\n\nmodel Lists {\n  id          Int           @id @default(autoincrement())\n  name        String        @unique\n  description String?\n  isMain      Boolean       @default(false) @map(\"is_main\")\n  games       ListEntries[]\n}\n\nmodel ListEntries {\n  id       Int     @id @default(autoincrement())\n  gameId   Int     @map(\"game_id\")\n  listId   Int     @map(\"list_id\")\n  notes    String?\n  position Int\n\n  game    Games @relation(fields: [gameId], references: [id])\n  lists   Lists @relation(fields: [listId], references: [id])\n  listsId Int?\n\n  @@unique([gameId, listId])\n  @@index([position])\n}\n\nmodel Settings {\n  id                Int     @id @default(autoincrement())\n  displayedMetadata Int     @default(0) @map(\"display_metadata\")\n  metadata1Label    String  @default(\"Expectation\") @map(\"metadata1_label\")\n  metadata1Presets  String? @default(\",1 - Must Play,2 - Highly Interested,3 - Moderate Interest,4 - Interested,5 - Worth a Shot,6 - Series Pending,\") @map(\"metadata1_presets\")\n  metadata2Status   Boolean @default(false) @map(\"metadata2_status\")\n  metadata2Label    String  @default(\"Metadata 2\") @map(\"metadata2_label\")\n  metadata2Presets  String? @map(\"metadata2_presets\")\n  metadata3Status   Boolean @default(false) @map(\"metadata3_status\")\n  metadata3Label    String  @default(\"Metadata 3\") @map(\"metadata3_label\")\n  metadata3Presets  String? @map(\"metadata3_presets\")\n  metadata4Status   Boolean @default(false) @map(\"metadata4_status\")\n  metadata4Label    String  @default(\"Metadata 4\") @map(\"metadata4_label\")\n  metadata4Presets  String? @map(\"metadata4_presets\")\n  metadata5Status   Boolean @default(false) @map(\"metadata5_status\")\n  metadata5Label    String  @default(\"Metadata 5\") @map(\"metadata5_label\")\n  metadata5Presets  String? @map(\"metadata5_presets\")\n}\n",
	"runtimeDataModel": {
		"models": {},
		"enums": {},
		"types": {}
	}
};
config.runtimeDataModel = JSON.parse("{\"models\":{\"Games\":{\"fields\":[{\"name\":\"id\",\"kind\":\"scalar\",\"type\":\"Int\"},{\"name\":\"name\",\"kind\":\"scalar\",\"type\":\"String\"},{\"name\":\"cover\",\"kind\":\"scalar\",\"type\":\"String\"},{\"name\":\"status\",\"kind\":\"scalar\",\"type\":\"String\"},{\"name\":\"genres\",\"kind\":\"scalar\",\"type\":\"String\"},{\"name\":\"length\",\"kind\":\"scalar\",\"type\":\"Int\"},{\"name\":\"platform\",\"kind\":\"scalar\",\"type\":\"String\"},{\"name\":\"reviewScore\",\"kind\":\"scalar\",\"type\":\"Float\",\"dbName\":\"review_score\"},{\"name\":\"metadata1\",\"kind\":\"scalar\",\"type\":\"String\"},{\"name\":\"metadata2\",\"kind\":\"scalar\",\"type\":\"String\"},{\"name\":\"metadata3\",\"kind\":\"scalar\",\"type\":\"String\"},{\"name\":\"metadata4\",\"kind\":\"scalar\",\"type\":\"String\"},{\"name\":\"metadata5\",\"kind\":\"scalar\",\"type\":\"String\"},{\"name\":\"listsEntries\",\"kind\":\"object\",\"type\":\"ListEntries\",\"relationName\":\"GamesToListEntries\"}],\"dbName\":null},\"Lists\":{\"fields\":[{\"name\":\"id\",\"kind\":\"scalar\",\"type\":\"Int\"},{\"name\":\"name\",\"kind\":\"scalar\",\"type\":\"String\"},{\"name\":\"description\",\"kind\":\"scalar\",\"type\":\"String\"},{\"name\":\"isMain\",\"kind\":\"scalar\",\"type\":\"Boolean\",\"dbName\":\"is_main\"},{\"name\":\"games\",\"kind\":\"object\",\"type\":\"ListEntries\",\"relationName\":\"ListEntriesToLists\"}],\"dbName\":null},\"ListEntries\":{\"fields\":[{\"name\":\"id\",\"kind\":\"scalar\",\"type\":\"Int\"},{\"name\":\"gameId\",\"kind\":\"scalar\",\"type\":\"Int\",\"dbName\":\"game_id\"},{\"name\":\"listId\",\"kind\":\"scalar\",\"type\":\"Int\",\"dbName\":\"list_id\"},{\"name\":\"notes\",\"kind\":\"scalar\",\"type\":\"String\"},{\"name\":\"position\",\"kind\":\"scalar\",\"type\":\"Int\"},{\"name\":\"game\",\"kind\":\"object\",\"type\":\"Games\",\"relationName\":\"GamesToListEntries\"},{\"name\":\"lists\",\"kind\":\"object\",\"type\":\"Lists\",\"relationName\":\"ListEntriesToLists\"},{\"name\":\"listsId\",\"kind\":\"scalar\",\"type\":\"Int\"}],\"dbName\":null},\"Settings\":{\"fields\":[{\"name\":\"id\",\"kind\":\"scalar\",\"type\":\"Int\"},{\"name\":\"displayedMetadata\",\"kind\":\"scalar\",\"type\":\"Int\",\"dbName\":\"display_metadata\"},{\"name\":\"metadata1Label\",\"kind\":\"scalar\",\"type\":\"String\",\"dbName\":\"metadata1_label\"},{\"name\":\"metadata1Presets\",\"kind\":\"scalar\",\"type\":\"String\",\"dbName\":\"metadata1_presets\"},{\"name\":\"metadata2Status\",\"kind\":\"scalar\",\"type\":\"Boolean\",\"dbName\":\"metadata2_status\"},{\"name\":\"metadata2Label\",\"kind\":\"scalar\",\"type\":\"String\",\"dbName\":\"metadata2_label\"},{\"name\":\"metadata2Presets\",\"kind\":\"scalar\",\"type\":\"String\",\"dbName\":\"metadata2_presets\"},{\"name\":\"metadata3Status\",\"kind\":\"scalar\",\"type\":\"Boolean\",\"dbName\":\"metadata3_status\"},{\"name\":\"metadata3Label\",\"kind\":\"scalar\",\"type\":\"String\",\"dbName\":\"metadata3_label\"},{\"name\":\"metadata3Presets\",\"kind\":\"scalar\",\"type\":\"String\",\"dbName\":\"metadata3_presets\"},{\"name\":\"metadata4Status\",\"kind\":\"scalar\",\"type\":\"Boolean\",\"dbName\":\"metadata4_status\"},{\"name\":\"metadata4Label\",\"kind\":\"scalar\",\"type\":\"String\",\"dbName\":\"metadata4_label\"},{\"name\":\"metadata4Presets\",\"kind\":\"scalar\",\"type\":\"String\",\"dbName\":\"metadata4_presets\"},{\"name\":\"metadata5Status\",\"kind\":\"scalar\",\"type\":\"Boolean\",\"dbName\":\"metadata5_status\"},{\"name\":\"metadata5Label\",\"kind\":\"scalar\",\"type\":\"String\",\"dbName\":\"metadata5_label\"},{\"name\":\"metadata5Presets\",\"kind\":\"scalar\",\"type\":\"String\",\"dbName\":\"metadata5_presets\"}],\"dbName\":null}},\"enums\":{},\"types\":{}}");
async function decodeBase64AsWasm(wasmBase64) {
	const { Buffer: Buffer$1 } = await import("node:buffer");
	const wasmArray = Buffer$1.from(wasmBase64, "base64");
	return new WebAssembly.Module(wasmArray);
}
config.compilerWasm = {
	getRuntime: async () => await import("@prisma/client/runtime/query_compiler_bg.sqlite.mjs"),
	getQueryCompilerWasmModule: async () => {
		const { wasm } = await import("@prisma/client/runtime/query_compiler_bg.sqlite.wasm-base64.mjs");
		return await decodeBase64AsWasm(wasm);
	}
};
function getPrismaClientClass() {
	return __prisma_client_runtime_client.getPrismaClient(config);
}
const PrismaClientKnownRequestError = __prisma_client_runtime_client.PrismaClientKnownRequestError;
const PrismaClientUnknownRequestError = __prisma_client_runtime_client.PrismaClientUnknownRequestError;
const PrismaClientRustPanicError = __prisma_client_runtime_client.PrismaClientRustPanicError;
const PrismaClientInitializationError = __prisma_client_runtime_client.PrismaClientInitializationError;
const PrismaClientValidationError = __prisma_client_runtime_client.PrismaClientValidationError;
const sql = __prisma_client_runtime_client.sqltag;
const empty = __prisma_client_runtime_client.empty;
const join$2 = __prisma_client_runtime_client.join;
const raw = __prisma_client_runtime_client.raw;
const Sql = __prisma_client_runtime_client.Sql;
const Decimal = __prisma_client_runtime_client.Decimal;
const getExtensionContext = __prisma_client_runtime_client.Extensions.getExtensionContext;
const NullTypes = {
	DbNull: __prisma_client_runtime_client.NullTypes.DbNull,
	JsonNull: __prisma_client_runtime_client.NullTypes.JsonNull,
	AnyNull: __prisma_client_runtime_client.NullTypes.AnyNull
};
const DbNull = __prisma_client_runtime_client.DbNull;
const JsonNull = __prisma_client_runtime_client.JsonNull;
const AnyNull = __prisma_client_runtime_client.AnyNull;
const TransactionIsolationLevel = __prisma_client_runtime_client.makeStrictEnum({ Serializable: "Serializable" });
const defineExtension = __prisma_client_runtime_client.Extensions.defineExtension;
globalThis["__dirname"] = node_path.dirname((0, node_url.fileURLToPath)(require("url").pathToFileURL(__filename).href));
const PrismaClient = getPrismaClientClass();
const getPaths = () => {
	const documentsPath = electron.app.getPath("documents");
	const folderPath = node_path.default.join(documentsPath, "backloggy");
	return {
		documentsPath,
		folderPath,
		databasePath: node_path.default.join(folderPath, "games.db")
	};
};
const nullToUndefined = (value) => {
	return value === null ? void 0 : value;
};
const stringToArray = (text) => {
	return text.split(",").filter(Boolean);
};
const arrayToString = (array) => {
	return `,${array.join(",")},`;
};
function ensureErrorMessage(value) {
	if (value instanceof Error) return value;
	let stringified = "[Unable to stringify the thrown value]";
	try {
		stringified = JSON.stringify(value);
	} catch {}
	return (/* @__PURE__ */ new Error(`This value was throw as is, not through an Error: ${stringified}`)).message;
}
var { databasePath } = getPaths();
var adapter = new __prisma_adapter_better_sqlite3.PrismaBetterSqlite3({ url: databasePath });
const prisma = new PrismaClient({ adapter });
const GAME_PLATFORM_SUGGESTIONS = [
	"PC",
	"NES",
	"SNES",
	"N64",
	"GameCube",
	"Wii",
	"Wii U",
	"Switch",
	"Switch 2",
	"GB",
	"GBC",
	"GBA",
	"DS",
	"3DS",
	"VB",
	"PS1",
	"PS2",
	"PS3",
	"PS4",
	"PS5",
	"PSP",
	"PSVita",
	"Genesis",
	"Game Gear",
	"Sega CD",
	"Saturn",
	"Dreamcast",
	"Xbox",
	"Xbox 360",
	"Xbox One",
	"3DO",
	"PC-88",
	"PC-98",
	"WonderSwan",
	"Mobile",
	"PCE CD",
	"Neo Geo",
	"TG-16"
];
const GAME_STATUS_SUGGESTIONS = [
	"Not Released",
	"Early Access",
	"Region Locked",
	"Backlog",
	"Playing",
	"Completed",
	"100%",
	"Dropped",
	"DLC"
];
const GAME_STATUS = [...GAME_STATUS_SUGGESTIONS];
const GAME_PLATFORMS = [...GAME_PLATFORM_SUGGESTIONS];
const metadataKeys = [
	"metadata1",
	"metadata2",
	"metadata3",
	"metadata4",
	"metadata5"
];
const DisplayedMetadataSchema = zod.default.object({ displayedMetadata: zod.default.number().nullable().default(0) }).transform((metadata) => ({ displayedMetadata: metadataKeys[metadata.displayedMetadata ?? 0] }));
const GameCardSchema = zod.default.array(zod.default.object({
	position: zod.default.number(),
	game: zod.default.object({
		id: zod.default.number(),
		name: zod.default.string(),
		cover: zod.default.url(),
		genres: zod.default.string(),
		platform: zod.default.enum(GAME_PLATFORMS),
		status: zod.default.enum(GAME_STATUS),
		length: zod.default.number().nullable().optional(),
		reviewScore: zod.default.number().nullable().optional(),
		metadata1: zod.default.string().nullable().optional(),
		metadata2: zod.default.string().nullable().optional(),
		metadata3: zod.default.string().nullable().optional(),
		metadata4: zod.default.string().nullable().optional(),
		metadata5: zod.default.string().nullable().optional()
	})
}).transform(({ game, position }) => {
	const { genres, length, metadata1, metadata2, metadata3, metadata4, metadata5, reviewScore, ...otherProperties } = game;
	return {
		...otherProperties,
		genres: stringToArray(genres),
		length: nullToUndefined$1(length),
		metadata: metadata1 ? nullToUndefined$1(metadata1) : metadata2 ? nullToUndefined$1(metadata2) : metadata3 ? nullToUndefined$1(metadata3) : metadata4 ? nullToUndefined$1(metadata4) : nullToUndefined$1(metadata5),
		reviewScore: nullToUndefined$1(reviewScore),
		position
	};
}));
function nullToUndefined$1(value) {
	return value === null ? void 0 : value;
}
const GameEditSchema = zod.default.object({
	id: zod.default.number(),
	name: zod.default.string(),
	cover: zod.default.string(),
	genres: zod.default.string(),
	platform: zod.default.string(),
	status: zod.default.string(),
	length: zod.default.number().nullable().optional(),
	reviewScore: zod.default.number().nullable().optional(),
	metadata1: zod.default.string().nullable().optional(),
	metadata2: zod.default.string().nullable().optional(),
	metadata3: zod.default.string().nullable().optional(),
	metadata4: zod.default.string().nullable().optional(),
	metadata5: zod.default.string().nullable().optional(),
	listsEntries: zod.default.array(zod.default.object({
		position: zod.default.number(),
		notes: zod.default.string().nullable().optional()
	}))
}).transform(({ genres, length, metadata1, metadata2, metadata3, metadata4, metadata5, listsEntries, status, reviewScore, platform, ...otherProperties }) => ({
	...otherProperties,
	status,
	platform,
	reviewScore: nullToUndefined(reviewScore),
	genres: stringToArray(genres),
	length: nullToUndefined(length),
	metadata1: nullToUndefined(metadata1),
	metadata2: nullToUndefined(metadata2),
	metadata3: nullToUndefined(metadata3),
	metadata4: nullToUndefined(metadata4),
	metadata5: nullToUndefined(metadata5),
	position: listsEntries[0].position,
	notes: nullToUndefined(listsEntries[0].notes)
}));
const fetchBacklogGameById = async (id) => {
	const game = await prisma.games.findFirst({
		where: { id },
		include: { listsEntries: { where: { lists: { isMain: true } } } }
	});
	return GameEditSchema.parse(game);
};
const fetchBacklogGameCount = async () => {
	return await prisma.games.count({ where: { listsEntries: { every: { lists: { isMain: true } } } } });
};
const createBacklogOrderBy = (sort) => {
	if (!sort || !sort.option || sort.option === "position") return { position: sort?.order ?? "asc" };
	return { game: { [sort.option]: sort.order || "asc" } };
};
const createBacklogSelect = (metadata) => {
	return {
		position: true,
		game: { select: {
			id: true,
			cover: true,
			name: true,
			genres: true,
			length: true,
			platform: true,
			status: true,
			reviewScore: true,
			[metadata]: true
		} }
	};
};
const createBacklogWhere = (filters, search) => {
	if (!filters && !search) return void 0;
	const where = {};
	const { length, genres, ...otherProperties } = filters ?? {};
	if (search) where.name = { contains: search };
	for (const [key, value] of Object.entries(otherProperties)) if (value !== void 0) where[key] = value;
	if (genres?.length) where.AND = genres.map((genre) => ({ genres: { contains: `,${genre},` } }));
	const hasMin = length?.min !== void 0;
	const hasMax = length?.max !== void 0;
	if (length && (hasMin || hasMax)) where.length = {
		...hasMin && { gte: length.min },
		...hasMax && { lte: length.max }
	};
	return where;
};
const fetchBacklogGames = async ({ search, filters, sort, currentPage, itemsPerPage }, metadata) => {
	const games = await prisma.listEntries.findMany({
		where: { game: createBacklogWhere(filters, search) },
		select: createBacklogSelect(metadata),
		orderBy: createBacklogOrderBy(sort),
		skip: itemsPerPage * (currentPage - 1),
		take: itemsPerPage
	});
	return GameCardSchema.parse(games);
};
const fetchDisplayedMetadata = async () => {
	const settings = await prisma.settings.findFirst({ select: { displayedMetadata: true } });
	return DisplayedMetadataSchema.parse(settings);
};
const updateBacklogGame = async (id, { genres, ...gameToEdit }) => {
	await prisma.games.update({
		where: { id },
		data: {
			genres: arrayToString(genres),
			...gameToEdit
		}
	});
};
const decrementGamesBacklogPosition = async (tx, oldPosition, newPosition) => {
	if (newPosition <= oldPosition) return;
	await tx.listEntries.updateMany({
		where: {
			lists: { isMain: true },
			position: {
				gt: oldPosition,
				lte: newPosition
			}
		},
		data: { position: { decrement: 1 } }
	});
};
const incrementGamesBacklogPosition = async (tx, oldPosition, newPosition) => {
	if (newPosition >= oldPosition) return;
	await tx.listEntries.updateMany({
		where: {
			lists: { isMain: true },
			position: {
				gte: newPosition,
				lt: oldPosition
			}
		},
		data: { position: { increment: 1 } }
	});
};
const updateBacklogPosition = async (gameId, newPosition) => {
	return prisma.$transaction(async (tx) => {
		const game = await tx.listEntries.findFirst({ where: {
			gameId,
			lists: { isMain: true }
		} });
		if (!game) throw new Error("Game not found!");
		const oldPosition = game.position;
		incrementGamesBacklogPosition(tx, oldPosition, newPosition);
		decrementGamesBacklogPosition(tx, oldPosition, newPosition);
		await tx.listEntries.updateMany({
			where: {
				gameId,
				lists: { isMain: true }
			},
			data: { position: newPosition }
		});
	});
};
const updateBacklogGameNotes = async (gameId, notes) => {
	await prisma.listEntries.updateMany({
		where: {
			gameId,
			lists: { isMain: true }
		},
		data: { notes }
	});
};
const getBacklogGames = async (queryOptions) => {
	const { displayedMetadata } = await fetchDisplayedMetadata();
	return await fetchBacklogGames(queryOptions, displayedMetadata);
};
const editBacklogGame = async ({ id, position, notes, ...game }) => {
	try {
		await updateBacklogPosition(id, position);
		await updateBacklogGameNotes(id, notes);
		await updateBacklogGame(id, game);
	} catch (error) {
		console.error(`Error editing backlog entry: ${ensureErrorMessage(error)}`);
	}
};
const getBacklogGameById = async (id) => {
	return await fetchBacklogGameById(id);
};
const getBacklogPageAmount = async (itemsPerPage = 30) => {
	const totalItemCount = await fetchBacklogGameCount();
	return Math.ceil(totalItemCount / itemsPerPage);
};
const handleBacklogIPC = () => {
	electron.ipcMain.handle("get-backlog-games", async (_event, queryOptions) => {
		return getBacklogGames(queryOptions);
	});
	electron.ipcMain.handle("get-backlog-page-amount", async (_event, itemsPerPage = 30) => {
		return getBacklogPageAmount(itemsPerPage);
	});
	electron.ipcMain.handle("get-backlog-game-by-id", async (_event, id) => {
		return getBacklogGameById(id);
	});
	electron.ipcMain.handle("edit-backlog-game", async (_event, game) => {
		return editBacklogGame(game);
	});
};
const MAX_DISPLAYED_METADATA = 4;
const SettingsSchema = zod.default.object({
	id: zod.default.number(),
	displayedMetadata: zod.default.number(),
	metadata1Label: zod.default.string(),
	metadata1Presets: zod.default.string().nullable(),
	metadata2Status: zod.default.boolean(),
	metadata2Label: zod.default.string(),
	metadata2Presets: zod.default.string().nullable(),
	metadata3Status: zod.default.boolean(),
	metadata3Label: zod.default.string(),
	metadata3Presets: zod.default.string().nullable(),
	metadata4Status: zod.default.boolean(),
	metadata4Label: zod.default.string(),
	metadata4Presets: zod.default.string().nullable(),
	metadata5Status: zod.default.boolean(),
	metadata5Label: zod.default.string(),
	metadata5Presets: zod.default.string().nullable()
}).transform(({ displayedMetadata, metadata1Label, metadata1Presets, metadata2Label, metadata2Presets, metadata2Status, metadata3Label, metadata3Presets, metadata3Status, metadata4Label, metadata4Presets, metadata4Status, metadata5Label, metadata5Presets, metadata5Status }) => {
	return {
		displayedMetadata: displayedMetadata <= 4 ? displayedMetadata : 0,
		metadata1: {
			label: metadata1Label,
			presets: metadata1Presets ? stringToArray(metadata1Presets) : void 0
		},
		metadata2: {
			status: metadata2Status,
			label: metadata2Label,
			presets: metadata2Presets ? stringToArray(metadata2Presets) : void 0
		},
		metadata3: {
			status: metadata3Status,
			label: metadata3Label,
			presets: metadata3Presets ? stringToArray(metadata3Presets) : void 0
		},
		metadata4: {
			status: metadata4Status,
			label: metadata4Label,
			presets: metadata4Presets ? stringToArray(metadata4Presets) : void 0
		},
		metadata5: {
			status: metadata5Status,
			label: metadata5Label,
			presets: metadata5Presets ? stringToArray(metadata5Presets) : void 0
		}
	};
});
const fetchSettings = async () => {
	const settings = await prisma.settings.findFirst();
	return SettingsSchema.parse(settings);
};
const getSettings = async () => {
	return await fetchSettings();
};
const handleSettingsIPC = () => {
	electron.ipcMain.handle("get-settings", getSettings);
};
const handleIPC = () => {
	handleBacklogIPC();
	handleSettingsIPC();
};
const exitOnAllWindowsClose = () => {
	electron.app.on("window-all-closed", () => {
		if (process.platform === "darwin") return;
		electron.app.quit();
	});
};
var icon_default = "data:image/png;base64,";
const createMainWindow = () => {
	return new electron.BrowserWindow({
		width: 900,
		height: 670,
		darkTheme: true,
		show: false,
		autoHideMenuBar: true,
		...process.platform === "linux" ? { icon: icon_default } : {},
		webPreferences: {
			preload: (0, node_path.join)(__dirname, "../preload/index.js"),
			sandbox: false
		}
	});
};
const loadWindowRenderer = (mainWindow) => {
	const isDev = __electron_toolkit_utils.is.dev;
	const rendererURL = process.env["ELECTRON_RENDERER_URL"];
	if (isDev && rendererURL) {
		mainWindow.loadURL(rendererURL);
		return;
	}
	mainWindow.loadFile((0, node_path.join)(__dirname, "../renderer/index.html"));
};
const setupExternalLinksHandler = (mainWindow) => {
	mainWindow.webContents.setWindowOpenHandler((details) => {
		electron.shell.openExternal(details.url);
		return { action: "deny" };
	});
};
const showMainWindow = (mainWindow) => {
	mainWindow.on("ready-to-show", () => {
		mainWindow.show();
	});
};
const initializeMainWindow = () => {
	const mainWindow = createMainWindow();
	showMainWindow(mainWindow);
	setupExternalLinksHandler(mainWindow);
	loadWindowRenderer(mainWindow);
};
electron.app.whenReady().then(() => {
	__electron_toolkit_utils.electronApp.setAppUserModelId("com.electron");
	electron.app.on("browser-window-created", (_, window) => {
		__electron_toolkit_utils.optimizer.watchWindowShortcuts(window);
	});
	handleIPC();
	initializeMainWindow();
	electron.app.on("activate", () => {
		if (electron.BrowserWindow.getAllWindows().length === 0) initializeMainWindow();
	});
});
exitOnAllWindowsClose();
